...
createApp({
    ...
    methods: {
        ...
        async handleClick(keyword, code, event) {
            ...
            // 서버와 통신하여 데이터를 가져옴
            await this.someFunction(keyword, code, popoverElement);
        },
        async someFunction(keyword, code, popoverElement) {
            try {
                const response = await fetch('/culture/api/value', {
                    method: 'POST',
                    headers: {
